...
def create_user(name, email, age):
    ...
    try:
        ...
        # Debugging line: Check your terminal running Streamlit to see the raw output!
        print(f"[DEBUG] Status Code: {response.status_code} | Response Text: {response.text}")
...
#         st.error(" cannot connect to FastAPI server. Please make user it's running on http://localhost:8001")
#         st.info("run: 'python fastapi_mongo.py' to start the sever")
#         return
            ...
            for post in posts:
                with st.expander(f"📝 {post['title']} (ID: {post['id'][:8]}...)"):
                    ...
                    with col1:
                        ...
                        created_time = pd.to_datetime(post['created_at']).strftime('%Y-%m-%d %H:%M:%S')
                        st.write(f"**Created:** {created_time}")
                    with col2:
                        st.write(f"**User ID:** {post['user_id'][:8]}...")
                        if st.button("Delete", key=f"delete_post_{post['id']}", type="secondary"):
    ...
    if users_success and posts_success:
        ...
        if posts:
            ...
            for post in recent_posts:
                formatted_date = pd.to_datetime(post['created_at']).strftime('%Y-%m-%d %H:%M:%S')
                st.write(f"• **{post['title']}** - {formatted_date}")
    else:
...
def main():
    ...
    if not check_api_connection():
        st.error(f"❌ Cannot connect to FastAPI server. Please make sure it's running on {API_BASE_URL}")
        st.info("Run: `uvicorn main:app --reload --port 8001` to start the server")
        return
